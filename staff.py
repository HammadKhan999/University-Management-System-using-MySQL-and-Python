    ...
    if count!=1:
     if id=="" or first_name=="" or last_name=="" :
         ...
         conn = pyodbc.connect('Driver={SQL Server};'
                              ...
                              'Trusted_Connection=yes;')
         cursor = conn.cursor()
         cursor.execute("insert into staff values('"+str(id)+"','"+str(first_name)+"','"+str(last_name)+"','"+str(designation)+"','"+str(department)+"','"+str(address)+"','"+str(phone)+"')")
         conn.commit()
...
def delete():
    ...
    if enter_id.get() == "":
         ...
         conn = pyodbc.connect('Driver={SQL Server};'
                               ...
                               'Trusted_Connection=yes;')
         cursor = conn.cursor()
         cursor.execute("delete from staff where (staff_id='"+ str(id) +"')")
         conn.commit()
...
def update():
    ...
    designation = enter_des.get();
    phone = enter_ph.get();
    if id == "" or first_name == "" or last_name == "" or address == "" or department == "" or designation== "" or phone=="":
        ...
        conn = pyodbc.connect('Driver={SQL Server};'
                              ...
                              'Trusted_Connection=yes;')
        cursor = conn.cursor()
        cursor.execute("UPDATE staff SET first_name='" + str(first_name) + "',last_name='" + str(
            last_name) + "',department='" + str(department) + "',addres='" + str(address) + "',designation='" + str(
            designation) + "', phone_no='"+str(phone)+"' where staff_id='" + str(id) + "'")
...
def Search():
    ...
    conn = pyodbc.connect('Driver={SQL Server};'
                          ...
                          'Trusted_Connection=yes;')
    cursor=conn.cursor()
    cursor.execute("SELECT * FROM staff WHERE (staff_id='" + str(id) + "')")
    for data in cursor:
            trv.insert("", 'end', values=(data[0], data[1], data[2], data[3], data[4], data[5],data[6]))
    conn.close()
root=Tk()
...
trv.heading("7",text="Phone Number")
...
left = Label(topframe, text="STAFF PORTAL ",bg='blue',fg='white')
left.pack()
...
ph=Label(leftframe,text='Phone number',font=('bold',10))
ph.place(x=6,y=250)
enter_ph=Entry()
enter_ph.place(x=150,y=350)
